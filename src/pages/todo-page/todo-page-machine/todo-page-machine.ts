...
/**
 * ~ "atomic state" - обикновен "еднослоен" state.
 * ~ "compound state" - state, който е многослоен, т.е. има вложен state в главният state (нестинг).
 * ~ "parallel state" - като compound state-а е, но може да изпълнява няколко неща едновременно.
 *      Паралелното състояние няма "initial state".
 * ~ Вторият аргумент в createMachine може да се пише не тук в машината, в а компонента, където викаме
 *    useMachine. Ако ги имаме и на двеме места превес взима този в useMachine.
 * ~ state.can(<action string>) връща Boolean. Така можем да проверяваме дали дадена транзиция е възможна или не.
 * ~ state.hasTag(<tag string>) връща Boolean. Можем да добавяме тагове в различните "states" и после да проверяваме
 *    за тях в компонента. Вършат работа като флагове. Пример: Ако имаме микрофон и сме заглушени проверяваме със
 *    state.hasTag("Mic is muted") и ако върне true показваме зелена микрофон икона, за да знае потребителя, че ако
 *    кликне ще се отглуши.
 * ~ Имената "actions" в "states" може да се пишат в масив, за да се изпълнят всички наведнъж.
 * ~ "wildcard" типа се добавя на "state", който искаме да се изпълнява при абсолютно всеки преход (transition)
 *    в машината. Не е нужно да бъде връзван изрично с друг "state".
 */
